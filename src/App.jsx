...
function App() {
  const [messages, setMessages] = useState([
    { role: "assistant", content: "Salut! Je suis Kvel 🤖 Tape /image + ta description pour créer une image!" }
  ]);
  const [input, setInput] = useState("");
  const [thinking, setThinking] = useState(false);
  const bottomRef = useRef(null);

  const GROQ_KEY = import.meta.env.VITE_GROQ_API_KEY || "import.meta.env.VITE_GROQ_API_KEY";
  ...
  const sendMessage = async () => {
    ...
    try {
      if (userText.toLowerCase().startsWith("/image")) {
        ...
        await new Promise(r => setTimeout(r, 800)); // petit effet d'attente
  ...
  return (
    <div style={{ background: "#111b21", height: "100vh", display: "flex", justifyContent: "center", fontFamily: "Segoe UI, sans-serif" }}>
      <div style={{ width: "100%", maxWidth: "480px", background: "#e5ddd5", display: "flex", flexDirection: "column", position: "relative" }}>
        ...
        {/* CHAT */}
        <div style={{ flex: 1, overflowY: "auto", padding: "16px", backgroundImage: "url('https://user-images.githubusercontent.com/15075759/28719144-86dc0f70-73b1-11e7-911d-60d70fcded21.png')", backgroundRepeat: "repeat" }}>
          {messages.map((m, i) => (
          ...
          {thinking && <div style={{ background: "white", padding: "8px 12px", borderRadius: "0 8px 8px 8px", width: "fit-content", fontSize: "13px" }}>en train d'écrire...</div>}
          <div ref={bottomRef} />
        </div>
        ...
        <div style={{ background: "#f0f2f5", padding: "8px", display: "flex", alignItems: "center", gap: "8px" }}>
          <div style={{ flex: 1, background: "white", borderRadius: "20px", display: "flex", alignItems: "center", padding: "6px 14px" }}>
            <input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&sendMessage()} placeholder="Message" style={{ flex: 1, border: "none", outline: "none", fontSize: "15px" }} />
          </div>
